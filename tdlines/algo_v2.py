...
def get_data_indexes(MAIN_DF_FILE):
    dates = []
    opens = []
    highs = []
    lows = []
    ...
    lows = lows[::-1]
    closes = closes[::-1]
    return dates, opens, highs, lows, closes, indexes


def get_td_points(indexes, highs, lows):
    td_highs = []
    td_lows = []
...
def td_lines_signal(revs_buy, td_lines, closes,revs_sell ):
    buy_signals = []
    sell_signals = []
...
df_tf = get_data(TF_CONFO_FILE, Skip_Bar_at_9PM)
...
dates, opens, highs, lows, closes, indexes = get_data_indexes(MAIN_DF_FILE)
td_highs, td_lows = get_td_points(indexes, highs, lows)
td_lines = get_td_lines(td_highs, td_lows, closes)
revs_buy, revs_sell = revs(indexes,dates,closes )
df_Sell_Signals, df_Buy_Signals = td_lines_signal(revs_buy, td_lines, closes,revs_sell )
...
if TF_CONFO:
    ...
    df_tf['RB_TF_Buy'] = df_tf.apply(
        lambda row: True if row['Close'] > row['Close_t-1'] and row['Close_t-1'] < row['Close_t-2'] else False, axis=1)
    df_tf['RB_TF_Sell'] = df_tf.apply(
        lambda row: True if row['Close'] < row['Close_t-1'] and row['Close_t-1'] > row['Close_t-2'] else False, axis=1)
    ...
    df_tf['TF_Confo_Buy'] = df_tf.apply(
        lambda row: True if row['RC_up_TF'] == True and row['RB_TF_Buy'] == True else False, axis=1)
    df_tf['TF_Confo_Sell'] = df_tf.apply(
        lambda row: True if row['RC_down_TF'] == True and row['RB_TF_Sell'] == True else False, axis=1)
...
df['L_POS'] = df.apply(
    lambda row: int((dollar_risk / 1000) / (row['Close'] - row['Low'])) if not np.isnan(row['BUY']) and row['Close'] !=
                                                                           row['Low'] else np.NaN, axis=1)
...
df['S_POS'] = df.apply(
    lambda row: int((dollar_risk / 1000) / (- row['Close'] + row['High'])) if not np.isnan(row['SELL']) and row[
...
df['TRADEx1'] = df.apply(lambda row: 'BUY' if row['TRADE'] == 'BUY' and np.isnan(row['L_POS_t-1']) else (
    'SELL' if row['TRADE'] == 'SELL' and np.isnan(row['S_POS_t-1']) else np.NaN), axis=1)

consecutive_trades = df.apply(
    lambda row: 1 if row['TRADE'] in ['BUY', 'SELL'] and row['EXIT'] in ['TP', 'STOP', 'NO HILO'] else 0, axis=1)
...
if PRINT_LINES:
    for bs in buy_signals:
        ...
        price_bs = bs[0][2]
        line = bs[1]
        data_first_TDP = line[0][0].strftime(
            "%d-%m-%Y %H:%M")  # convert date in dd/mm/yyyy hh:mm for first TDP for the line
        data_second_TDP = line[1][0].strftime("%d-%m-%Y %H:%M")
        price_first_TDP = line[0][2]
        price_second_TDP = line[1][2]
        slope_line = '{:.2f}'.format(tdl[2])
        print(date_bs + " BUY @ " + str(price_bs) + ' - line: ' + data_first_TDP + " @ " + (
                    str(price_first_TDP) + " / " + data_second_TDP + " @ " + str(price_second_TDP)))
    ...
    for ss in sell_signals:
        ...
        price_second_TDP = line[1][2]
        slope_line = '{:.2f}'.format(tdl[2])
        print(date_ss + " SELL @ " + str(price_ss) + ' - line: ' + data_first_TDP + " @ " + (
                    str(price_first_TDP) + " / " + data_second_TDP + " @ " + str(price_second_TDP)))
...
if PRINT_RESULTS:
    ...
    if (df.TRADE.count() - df.TRADEx1.count()) == 0:
        ...
        print('*** CHECK: 2nd singals when in pos...: ' + str(
            df.TRADE.count() - df.TRADEx1.count() - consecutive_trades.sum()))
